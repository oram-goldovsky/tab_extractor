...
def rotate_neck_picture(image):
    ...
    angle = median_slope * 45  # self note - mult by 45 because slope=1 iff angle=45degs

    return angle, Image(img=rotate(image_to_rotate, -angle))
...
def crop_neck_picture(image):
    ...
    # TODO: go over this calculation better, check for problems/ special cases. i.e what if there are 3 random lines at the bottom, will the results be garbage?
    # same problem occurs for extra lines at the top. possible alternative: check for overall density of lines
    ...
    for i in range(len(y_differences) - 1):
        if y_differences[i] == 0:
            last_y = y_sort[i]
            if i > 3 and first_y == 0:  # solves the problem mentioned in TODO? does it fail when the problem is double?(two separate lines at bottom and then the guitar)
                first_y = y_sort[i]

    return (first_y - 10), (last_y+10), Image(img=image_to_crop[first_y - 10:last_y + 10])
