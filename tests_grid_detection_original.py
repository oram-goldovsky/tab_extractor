...
def string_detection_tests():
    ...
    for filename in os.listdir('./pictures/'):
        ...
        _, rotated_image = rotate_neck_picture(chord_image)
        _, _, cropped_image = crop_neck_picture(rotated_image)
        # original output
        # neck_string = string_detection(cropped_image)[1]
        # original output
        # TODO: my code testing 2
        # check_strings_output = string_detection(cropped_image)[0]
        # string_edges_cropped_rotated = string_detection(cropped_image)[0].separating_lines
        neck_string = string_detection(cropped_image)[1]
        # TODO: my code testing 2 end
...
def fret_detection_tests():
    ...
    for filename in os.listdir('./pictures/'):
        ...
        _, rotated_image = rotate_neck_picture(chord_image)
        _, _, cropped_image = crop_neck_picture(rotated_image)
        _, neck_fret = fret_detection(cropped_image)
        plt.subplot(int("42" + str(i)))
        i += 1
        plt.imshow(cv2.cvtColor(chord_image.image, cv2.COLOR_BGR2RGB))
        plt.subplot(int("42" + str(i)))
